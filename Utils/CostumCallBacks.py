...
class PlotCallback(BaseCallback):
    ...
    """
    def __init__(self, episode_plot_freq: int = 10000, update_stats_every: int=1, average_size: int = 100, verbose=1, plot_prefix='quick_test', plot_dir="./Plots"):
        """
    ...
    def _on_training_start(self) -> bool:
        ...
        if self.training_env.curiosity:
            column_name = ['Episode', 'Time_step', 'Episode_timesteps', 'Mean_Episode_timesteps', 'Reward', 'Mean_Reward', 'CuriosityReward', 'Mean_curiosity_reward', 'Eps_greedy_val']
            self._init_plot_reward_curiosity()
        else:
            column_name = ['Episode', 'Time_step', 'Episode_timesteps', 'Mean_Episode_timesteps', 'Reward', 'Mean_Reward', 'Eps_greedy_val']
    ...
    def _on_step(self) -> bool:
        ...
        if self.training_env.done:
            ...
            if (self.episode_counter == self.update_stats_every) or (self.episode_counter == 0):
                self._update_df()
        ...
        if (self.model.num_timesteps % self.episode_plot_freq) == 0:
            ...
            if N > 0:
                ...
                self.episode_history_df.to_csv(self.plot_dir+'/'+self.plot_prefix+'_plot_data.out', index = False)
            else:
    ...
    def _update_df(self):
        ...
        #create mean values
        mean_reward = np.append(np.asarray(self.episode_history_df.Reward[-n_mean:]), self.episode_reward).mean()
        mean_ts = np.append(np.asarray(self.episode_history_df.Episode_timesteps[-n_mean:]), self.episode_timestep).mean()

        if self.training_env.curiosity:
            mean_reward_curiosity = np.append(np.asarray(self.episode_history_df.CuriosityReward[-n_mean:]), self.episode_rewardCuriosity).mean()

            #Update df
            self.episode_history_df = self.episode_history_df.append({'Episode':self.episode_num, 'Time_step':self.model.num_timesteps, 'Episode_timesteps':self.episode_timestep, 'Reward':self.episode_reward,'CuriosityReward':self.episode_rewardCuriosity, 'Eps_greedy_val':current_eps, 'Mean_Reward':mean_reward, 'Mean_curiosity_reward':mean_reward_curiosity, 'Mean_Episode_timesteps':mean_ts}, ignore_index = True)

            #Give format for memory efficiency
            self.episode_history_df['CuriosityReward'] = self.episode_history_df['CuriosityReward'].astype('float')
            self.episode_history_df['Mean_curiosity_reward'] = self.episode_history_df['Mean_curiosity_reward'].astype('float')
        else: #update df with no curiosity reward
            self.episode_history_df = self.episode_history_df.append({'Episode':self.episode_num, 'Time_step':self.model.num_timesteps, 'Episode_timesteps':self.episode_timestep, 'Reward':self.episode_reward,'Eps_greedy_val':current_eps, 'Mean_Reward':mean_reward, 'Mean_Episode_timesteps':mean_ts}, ignore_index = True)

        #Give format for memory efficiency
        self.episode_history_df['Episode'] = self.episode_history_df['Episode'].astype('uint32')
        self.episode_history_df['Time_step'] = self.episode_history_df['Time_step'].astype('uint32')
        self.episode_history_df['Episode_timesteps'] = self.episode_history_df['Episode_timesteps'].astype('uint16')
        self.episode_history_df['Reward'] = self.episode_history_df['Reward'].astype('float')
        self.episode_history_df['Eps_greedy_val'] = self.episode_history_df['Eps_greedy_val'].astype('float')
        self.episode_history_df['Mean_Reward'] = self.episode_history_df['Mean_Reward'].astype('float')
        self.episode_history_df['Mean_Episode_timesteps'] = self.episode_history_df['Mean_Episode_timesteps'].astype('float')

    def _plot_reward(self, df1, N):

        #The "if" is to avoid repeating plot legend
        self.ax_reward.plot(df1.Episode, df1.Reward, color = "blue", linewidth=.7, label='Reward' if self.legend_flag else "")
        ...
        self.ax_reward.plot(df1.Episode, df1.Mean_Reward, color="red", linewidth=.7, label=str(self.average_size)+' Rewar avg' if self.legend_flag else "")
        self.ax_reward.legend(bbox_to_anchor=(0, 1), loc='upper left')

        self.fig_reward.savefig(self.plot_dir+'/'+self.plot_prefix+'_mean_reward.png')
        del(df1)

    def _plot_reward_curiosity(self, df1, N):

        #The "if" is to avoid repeating plot legend
        self.ax_reward_c.plot(df1.Episode, df1.CuriosityReward, color="blue", linewidth=.7, label='CuriosityReward' if self.legend_flag else "")
        #plot dommy plot just to show Time step info in x axis
        self.ax_reward_c_2.plot(df1.Time_step, df1.Reward, linestyle='None')

        #Plot average of the last self.average_size observations
        self.ax_reward_c.plot(df1.Episode, df1.Mean_curiosity_reward, color="red", linewidth=.7, label=str(self.average_size)+' Rewar Cur avg' if self.legend_flag else "")
        ...
        self.fig_reward_c.savefig(self.plot_dir+'/'+self.plot_prefix+'_mean_curiosity_reward.png')
        del(df1)

    def _plot_epsilon(self, df1):

        #Min and Max Epsilon value lines
        self.ax_eps.axhline(y=self.model.exploration.initial_p, color='green', label='Initial Eps' if self.legend_flag else "")
        self.ax_eps.axhline(y=self.model.exploration.final_p, color='red', label='Final Eps' if self.legend_flag else "")
        #Epsilon plot
        self.ax_eps.plot(df1.Episode, df1['Eps_greedy_val'], color='blue', label='Eps' if self.legend_flag else "")
    ...
    def _plot_time_steps(self, df1, N):
        ...
        #The "if" is to avoid repeating plot legend
        self.ax_ts.plot(df1.Episode, df1.Episode_timesteps, color = "blue", linewidth=.7, label='Time steps' if self.legend_flag else "")

        #plot dommy plot just to show Time step info in x axis
        self.ax_ts_2.plot(df1.Time_step, df1.Episode_timesteps, linestyle='None')

        #Plot average of the last self.average_size observations
        self.ax_ts.plot(df1.Episode, df1.Mean_Episode_timesteps, color="red", linewidth=.7, label=str(self.average_size)+' Epi ts avg' if self.legend_flag else "")
        self.ax_ts.legend(bbox_to_anchor=(0, 1), loc='upper left')

        self.fig_ts.savefig(self.plot_dir+'/'+self.plot_prefix+'_episode_timesteps.png')
        del(df1)
    ...
    def _init_plot_reward(self):
        self.fig_reward, self.ax_reward= plt.subplots(nrows=1, ncols=1, figsize=(13, 6))
    ...
    def _init_plot_reward_curiosity(self):
        self.fig_reward_c, self.ax_reward_c= plt.subplots(nrows=1, ncols=1, figsize=(13, 6))
        # Add some extra space for the second axis at the bottom
