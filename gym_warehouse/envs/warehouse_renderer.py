...
class WarehouseRenderer:
    ...
    def render(self):
        ...
        pygame.image.save(surface, 'Surface/surface_'+str(self.surface_index)+'.PNG')
    ...
    def save_as_gif(self, fp_out="./Surface/animation.gif", sec_per_frame=0.5):
        ...
        img.save(fp=fp_out, format='GIF', append_images=ordered_imgs, save_all=True, duration=duration, optimize=False)
    ...
    def _draw_warehouse_object(self, warehouse_object, color, video=False):
        ...
        if not video:
            ...
            self.state[y : y + self.num_slots,
                       x : x + self.num_slots] = color / C_MAX

            for item in warehouse_object.status:
                if item != 0:
                    self.state[y,
                               x : x + self.num_slots] = C_ITEMS[item] / C_MAX
                y += 1
            ...
            if hasattr(warehouse_object, 'access_spots'):
                for access_spot in warehouse_object.access_spots:
                    ...
                    y, x = r * self.num_slots, c * self.num_slots
                    self.state[y : y + self.num_slots,
                               x : x + self.num_slots] = C_SPOT / C_MAX
    ...
    def _draw_panel(self):
        ...
        text_surface = font.render(f't = {self.env.time_step}/{self.env.time_limit}', True, WHITE)
    ...
    def save_state(self):
        ...
        """
        self.state = np.ones(((self.env.num_rows + 2) * self.num_slots,
        ...
        # Plot transactions
        y, x = (self.env.num_rows * self.num_slots,
                self.env.num_cols * self.num_slots)
        self.state[y : y + 2 * self.num_slots, 0 : x] = C_LEGEND / 30

        y, x = (int((self.env.num_rows + 0.5) * self.num_slots),
                2 * self.num_slots)
