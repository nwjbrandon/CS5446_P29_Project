...
class Curiosity2:
    def __init__(
        self, curiosity_vec, train_every=500, mem_capacity=50000, batch_size=32
    ):
        self.device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
        print(self.device)
        # sys.exit()

        self.curiosity_net = Multisoftmax_ffnn_small(
            np.sum(curiosity_vec), curiosity_vec[:-1]
        ).to(self.device)
    ...
    def train(self, current_state, action, next_state, optimizer, eta=1e-3):
        ...
        for i in range(len(pred_state)):
            ...
            if (
                np.argmax(pred_state[i].cpu().detach().numpy())
        ...
        if (self.train_counter % self.train_every) == 0 and (
            len(self.memory_target) >= self.batch_size
        ):
            ...
            for j in range(len(next_state)):
                ...
                prediction_batch_sub = torch.reshape(
                    prediction_batch_sub, (self.batch_size, -1)
                )

                target_batch = [
                    torch.from_numpy(np.array([self.memory_target[i][j]])).to(
                        self.device
                    )
                    for i in batch_mask
                ]
                ...
                loss_batch += F.cross_entropy(
                    input=prediction_batch_sub, target=target_batch
                )
...
class Curiosity:
    def __init__(
        ...
        self.curiosity_net = ffnn_1024x512x256x128(input_size, output_size).to(
            self.device
        )
        self.curiosity_net.apply(init_weights)
